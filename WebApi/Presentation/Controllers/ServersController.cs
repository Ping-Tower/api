...
[Route("api/servers")]
[Authorize]
public class ServersController : coo.Presentation.Common.Base.BaseApiController
{
    ...
    [HttpPut("{id}/settings")]
    public async Task<IActionResult> UpdateSettings(string id, [FromBody] UpdateSettingsCommand body, CancellationToken ct)
    {
        await _sender.Send(body with { ServerId = id }, ct);
        return NoContent();
    }
